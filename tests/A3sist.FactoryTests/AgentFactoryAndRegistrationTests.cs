...
using A3sist.FactoryTests.TestAgents;
...
namespace A3sist.FactoryTests
{
    /// <summary>
    /// Comprehensive tests for agent factory and registration system functionality
    /// </summary>
    public class AgentFactoryAndRegistrationTests : IDisposable
    {
        ...
        public AgentFactoryAndRegistrationTests()
        {
        ...
        [Fact]
        public async Task AgentFactory_CreateAgentByTypeName_Success()
        {
            // Arrange
            var typeName = typeof(MinimalTestAgent).AssemblyQualifiedName;
        ...
        [Fact]
        public async Task AgentFactory_GetRegisteredAgentNames_ReturnsRegisteredNames()
        {
            // Arrange
            await _agentFactory.RegisterAgentTypeAsync(typeof(MinimalTestAgent));
        ...
        [Fact]
        public async Task AgentDiscoveryService_DiscoverAllAgents_FindsTestAgent()
        {
            // Act
        ...
        public async Task AgentDiscoveryService_GetAgentMetadata_ReturnsCorrectMetadata()
        {
            ...
            Assert.Single(metadata.Capabilities);
        }

        [Fact]
        public async Task AgentDiscoveryService_GetAllAgentMetadata_ReturnsMetadataForAllAgents()
        {
            // Act
            ...
            Assert.Contains(metadataList, m => m.Name == "MinimalTestAgent");
        }
        ...
        [Fact]
        public async Task AgentDiscoveryService_ValidateInvalidAgentType_ReturnsInvalid()
        {
            // Act
        ...
        public async Task FullIntegration_DiscoverRegisterAndManage_Success()
        {
            ...
            // 8. Verify agent can handle requests
            var canHandle = await managedAgent.CanHandleAsync(request);
        ...
        [Fact]
        public async Task FactoryAndDiscovery_CompleteWorkflow_Success()
        {
            // 1. Discover agents in current assembly
            var discoveredTypes = await _discoveryService.DiscoverAgentsAsync(Assembly.GetExecutingAssembly());
            Assert.Contains(typeof(MinimalTestAgent), discoveredTypes);

            // 2. Validate discovered agent
            var validationResult = await _discoveryService.ValidateAgentTypeAsync(typeof(MinimalTestAgent));
            ...
            Assert.True(result.Success);
            Assert.Equal("Minimal test completed", result.Message);
